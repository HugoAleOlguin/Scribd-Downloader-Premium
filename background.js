 ...
 * @version 2.3.1
 * @description Fixed for Chrome MV3 (Service Worker + State Persistence)
 */
...
const StateManager = {
    get: async () => {
        try {
            ...
            return result.automationState || { tabId: null, docUrl: null, active: false };
        } catch (e) {
            return { tabId: null, docUrl: null, active: false };
        }
    },
    set: async (state) => {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "fetch_image") {
        fetch(request.url)
            .then(r => r.blob())
            .then(blob => { const reader = new FileReader(); reader.onloadend = () => sendResponse({ success: true, data: reader.result }); reader.readAsDataURL(blob); })
    ...
    if (request.action === "validate_download") {
        fetch(request.url, { method: 'HEAD' })
            .then(response => {
                ...
                const isValidSize = size > 2048; // > 2KB
    ...
    if (request.action === "open_external_downloader") {
        ...
        chrome.tabs.create({ url: targetUrl }, async (newTab) => {
            // Persist state for Service Worker lifecycle
        ...
        if (state.active && tabId === state.tabId) {
            ...
            chrome.scripting.executeScript({
                ...
                func: (urlToPaste) => {
                    ...
                    // --- UI RENDERER (Cleaned for injection) ---
                    ...
                    if (currentUrl.includes('ilide.info')) {
                        ...
                        const checkIframe = setInterval(() => {
                            ...
                            if (errorBar && errorBar.offsetParent !== null) {
                                ...
                                found = true;
                                updateStatusUI(4, "Error Detectado", "El visualizador remoto indica un fallo estructural.", "error");
                                return;
                            }
                                ...
                                try {
                                    ...
                                    if (urlParam) {
                                        ...
                                        chrome.runtime.sendMessage({ action: "validate_download", url: directPdfUrl }, (r) => {
                                            if (r && r.valid) { updateStatusUI(4, "¡Documento Validado!", "Descarga iniciada.", "success"); }
                                            else { updateStatusUI(4, "Archivo Dañado", "El PDF está vacío o corrupto.", "error"); }
                                        });
                                        return;
                                    }
                                } catch (e) { }
                            }
                            if (attempts > 30) { clearInterval(checkIframe); if (!found) updateStatusUI(4, "Tiempo Agotado", "El servidor no entregó el documento.", "error"); }
                        }, 1000);
                        return;
                    }
